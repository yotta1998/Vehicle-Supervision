...
#cap = cv2.VideoCapture(str(arguments[1]))
...
Eye_Rat=0
while True:
    #for each cycle print the counter and eye ratio
    print("EYE_RAT="+str(Eye_Rat)+"  "+"counter="+str(counter)+"  ")
    print(((Eye_Rat<0.25) and (counter>1)))
    null, frame = cap.read()
    ...
    for face in faces:
        ...
        #str(time.ctime(time.time())) // remeber for time setting
        if Eye_Rat < 0.25:
            counter=counter+1
            if ((Eye_Rat<0.25) and (counter>3)):
                print(time.ctime(time.time()) + "   " + str(time.time()))
                name=str(time.time())+".jpg"
                cv2.imwrite(name,frame)
                DATABASE.drowsiness_insertion(str(arguments[1]), name)
